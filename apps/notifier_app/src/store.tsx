...
interface AppContextType {
  ...
  createEvent: (
    event: Omit<AppEvent, 'id' | 'createdAt' | 'creatorId' | 'subscribers'>,
  ) => void;
  updateEvent: (
    eventId: string,
    data: Partial<Omit<AppEvent, 'id' | 'createdAt' | 'creatorId' | 'subscribers'>>,
  ) => void;
  triggerNotification: (
    eventId: string,
    message: string,
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const [quickTriggerEventId, setQuickTriggerEventId] = useState<string | null>(
    null,
  );

  const createEvent: AppContextType['createEvent'] = eventData => {
  ...
  const updateEvent: AppContextType['updateEvent'] = (eventId, data) => {
    setEvents(prev => prev.map(e => (e.id === eventId ? { ...e, ...data } : e)));
  };

  const triggerNotification: AppContextType['triggerNotification'] = (
    eventId,
    message,
    importance,
  ) => {
